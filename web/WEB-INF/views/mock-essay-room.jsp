...
<%@page import="com.hipzi.model.MockExamEssay"%>
...
<%
    ...
    List<MockExamEssay> examEssays = (List<MockExamEssay>) request.getAttribute("examEssays");
...
<body>
    ...
    <section class="exam-workspace" id="examWorkspace">
        ...
        <div class="exam-submit-overlay" id="examResultOverlay">
            <div class="exam-submit-card">
                ...
                <div class="exam-submit-summary" id="essayReferenceAnswers" style="background:transparent; padding:0; display: flex; flex-direction: column; gap: 1rem; text-align: left; max-height: 400px; overflow-y: auto;">
                    <!-- Reference answers will be injected here -->
                </div>
                <div class="exam-submit-actions">
    ...
    (function () {
        ...
        var examQuestions = [
        <% if (examEssays != null) {
            for (int i = 0; i < examEssays.size(); i++) {
                MockExamEssay essay = examEssays.get(i);
        %>
            {
                id: "<%= js(essay.getId()) %>",
                type: "essay",
                text: "<%= js(essay.getPromptText()) %>",
                referenceAnswer: "<%= js(essay.getReferenceAnswer()) %>"
            }<%= i + 1 < examEssays.size() ? "," : "" %>
        <%  }
        } %>
        ];
        ...
        function finishExam() {
            ...
            examSubmitBtn.textContent = 'Đã nộp';

            hideSubmitConfirmation();
            
            var answersHtml = '';
            examQuestions.forEach(function(q, index) {
                var refAns = q.referenceAnswer ? q.referenceAnswer : 'Không có đáp án tham khảo.';
                var userAns = answers[index] ? answers[index] : '(Bỏ trống)';
                
                answersHtml += '<div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px; padding:12px;">';
                answersHtml += '  <h4 style="margin: 0 0 8px 0; color:#0f172a; font-size:1rem;">Câu ' + (index + 1) + '</h4>';
                answersHtml += '  <p style="margin: 0 0 10px 0; color:#334155; font-size:0.9rem; font-weight:600;">' + q.text + '</p>';
                answersHtml += '  <div style="background:#fefce8; border-left:3px solid #facc15; padding:8px 12px; margin-bottom:10px; font-size:0.85rem;">';
                answersHtml += '    <strong style="color:#a16207; display:block; margin-bottom:4px;">Bài làm của bạn:</strong>';
                answersHtml += '    <div style="white-space:pre-wrap; color:#713f12;">' + userAns + '</div>';
                answersHtml += '  </div>';
                answersHtml += '  <div style="background:#f0fdf4; border-left:3px solid #4ade80; padding:8px 12px; font-size:0.85rem;">';
                answersHtml += '    <strong style="color:#166534; display:block; margin-bottom:4px;">Đáp án tham khảo:</strong>';
                answersHtml += '    <div style="white-space:pre-wrap; color:#14532d;">' + refAns + '</div>';
                answersHtml += '  </div>';
                answersHtml += '</div>';
            });
            
            document.getElementById('essayReferenceAnswers').innerHTML = answersHtml;
